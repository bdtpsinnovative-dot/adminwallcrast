...
export default function ImageGalleryPage() {
  ...
  // ✅ เปลี่ยนมาเรียกใช้ API Route สำหรับดึงรูป
  const fetchImages = async (isLoadMore = false) => {
    ...
    try {
      const response = await fetch(`/api/r2?limit=${PAGE_SIZE}&offset=${currentOffset}`);
  ...
  // ✅ เปลี่ยนมาเรียกใช้ API Route สำหรับลบรูป
...
const compressImage = async (file: File): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        ...
        // 🌟 ปรับความละเอียดสูงสุดเพิ่มขึ้นเป็น 1000px เพื่อให้ภาพคมชัด (เมื่อไฟล์ขยับเป็น 80KB)
        const MAX_SIZE = 1000; 
        ...
        let quality = 0.9; // เริ่มต้นที่คุณภาพสูง 90%
        let blob: Blob | null = null;
        
        // 🌟 เปลี่ยนเป้าหมายขนาดไฟล์เป็น 80KB ตามสั่งครับนาย
        const TARGET_SIZE_BYTES = 80 * 1024; 
        ...
        do {
          ...
          if (blob.size > TARGET_SIZE_BYTES) {
            quality -= 0.05; // ค่อยๆ ลดคุณภาพทีละนิดเพื่อให้ได้ขนาดที่ใกล้ 80KB ที่สุด
            
            if (quality < 0.4) {
              quality = 0.6;
              currentScale -= 0.1;
              if (currentScale < 0.3) break;
            }
          } else {
            break; 
          }
  ...
  // ✅ เปลี่ยนมาเรียกใช้ API Route สำหรับอัปโหลด
  ...
  // -------------------------
  // ส่วน UI ด้านล่างเหมือนเดิมเป๊ะครับนาย!
  // -------------------------
  return (
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800">คลังรูปสินค้า (R2 Cloudflare)</h1>
                ...
                {images.map((img) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div 
                        className="aspect-[4/3] bg-slate-50 relative overflow-hidden flex items-center justify-center p-2 cursor-pointer"
                        onClick={() => toggleSelection(img.name)} 
                      >
                        <img 
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              {uploadStatus === 'idle' && (
                ...
                >
                  ...
                  {replaceFileName ? (
                    <p className="text-xs text-orange-500 mt-2">⚠️ ไฟล์เดิมจะถูกเขียนทับทันที แต่ URL ในระบบจะยังใช้ได้เหมือนเดิม</p>
                  ) : (
  ...
  crop={crop}
  zoom={zoom}
  aspect={3 / 4} // 🌟 3 คือกว้าง, 4 คือสูง (รูปจะทรงสูงขึ้น)
  // หรือ aspect={2 / 3} ถ้าอยากให้สูงยาวกว่านี้อีกครับ
                ...
                <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">
                      {uploadStatus === 'compressing' ? 'กำลังบีบอัดภาพ (WebP)...' : 'กำลังอัปโหลดขึ้น Cloudflare R2...'}
                    </p>
